class SocketEdgeTTS {
	constructor(_indexpart, _filename, _filenum,
				...
				_statArea, _obj_threads_info, _save_to_var, _updateProgressCallback) {
		...
		this.updateProgressCallback = _updateProgressCallback // Store callback
	...
	onSocketClose() {
		...
		//if (this.mp3_saved && this.updateProgressCallback) {
		//	this.updateProgressCallback(this.indexpart);
		//}
		// Introduce a random delay before starting the next audio file
		const delay = Math.random() * 20; // Random delay up to 50 milliseconds
		setTimeout(() => {
	...
	save_mp3() {
		...
		if ( this.my_uint8Array.length > 0 ) {
			...
			// Removed this line: add_edge_tts(this.save_to_var)
			// Call the callback function to update progress immediately after saving
